...
매주 일요일 21:00 에 새 소재로 영상을 만들어 **비공개**로 업로드합니다. 서버와 스케줄러 두 개가 함께 떠 있어야 합니다.
...
- **조건** 탭 → `작업을 실행하기 위해 절전 모드 종료` 체크
  (일요일 21:00 에 PC 가 절전이면 실행되지 않습니다)
...
`스케줄러 시작 — 매주 sunday 21:00 에 실행합니다.` 와 `다음 실행 예정: ...` 이 찍히면 정상입니다.
...
- 스케줄러: 서버 미기동 시 재시도 후 실패 보고, 매주 일요일 21:00 다음 실행 시각 계산,
